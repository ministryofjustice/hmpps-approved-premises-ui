  ...
  describe('adminActions', () => {
    ...
    describe('if the status of the placement request is `matched`', () => {
      ...
      it('should return actions to amend a space booking', () => {
        ...
        const { adminActionsResult, actionAmendSpaceBooking } = setup({
          placementRequestDetail,
          permissions: [],
        })
        expect(adminActionsResult).toEqual([actionAmendSpaceBooking])
      })
      ...
      it('should return actions to amend and withdraw a booking if the status is `matched` and user has the withdraw permission ', () => {
        ...
        const { adminActionsResult, actionAmendSpaceBooking, actionWithdrawPlacement } = setup({
          placementRequestDetail,
          permissions: ['cas1_booking_withdraw'],
        })

        expect(adminActionsResult).toEqual([actionAmendSpaceBooking, actionWithdrawPlacement])
      })
    })
