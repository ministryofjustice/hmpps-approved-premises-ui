...
export const riskTabController = async ({ personService, token, crn }: TabControllerParameters): Promise<TabData> => {
  const [roshSummary, riskManagementPlan, offenceDetails, supportingInformation]: Array<Cas1OASysGroup> =
    await Promise.all([
