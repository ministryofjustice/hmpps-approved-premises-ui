...
import { placementRisksController, riskTabController } from './risk'
...
describe('risks', () => {
  ...
  describe('placementRisksController', () => {
    const placement = cas1SpaceBookingFactory.build({ id: 'placement-id' })
    const crn = 'crn'
    const riskToStaffSummary = faker.lorem.paragraph()
    const riskToResidentsSummary = faker.lorem.paragraph()

    const profileData: TaskData = {
      'risk-information': {
    ...
    it('should render the placement risks section', async () => {
      const result = await placementRisksController({
        crn,
        placement,
        profileData,
      })
      expect(result.cardList[0].card.title).toEqual({ text: 'Placement risks' })
      ...
      expect(result.cardList[0].card.actions.items[0]).toEqual({
        href: `/manage/resident/crn/placement/placement-id/tasks/profile/risk-information/page/risk-to-staff`,
        text: 'Change',
      })
    })

    it('should render an empty section if the pre-arrival tasks haven\t been completed', async () => {
      const result = await placementRisksController({
        crn,
        placement,
        profileData: {},
      })
      expect(result.cardList[0].html).toEqual(
        `This information will be available when the <a href="/manage/resident/crn/placement/placement-id/tasks/pre-arrival">pre-arrival tasks</a> are complete.`,
      )
      expect(result.cardList[0].card.actions).toBeUndefined()
    })
  })
})
