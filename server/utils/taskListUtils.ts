import { TaskData, TaskStatus as TaskListStatus, TaskWithStatus, UiTask } from '@approved-premises/ui'
...
export const taskLink = (
  ...
  if (task.status !== 'cannot_start') {
    const firstPage = Object.keys((data || applicationOrAssessment?.data)?.[task.id] || task.pages)[0]

    let link = `${pathRoot}/${task.id}/page/${firstPage}`
    if (applicationOrAssessment) {
      link = isAssessment(applicationOrAssessment)
