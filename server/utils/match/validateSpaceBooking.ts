...
export const validateSpaceBooking = (body: ObjectWithDateParts<string>): Record<string, string> => {
  ...
  if (datetimeIsInThePast(departureDate, arrivalDate)) {
    errors.departureDate = 'The departure date must not be before the arrival date'
  } else if (departureDate === arrivalDate) {
    errors.departureDate = 'The departure date and arrival date cannot be the same day'
  }
  return errors
}
