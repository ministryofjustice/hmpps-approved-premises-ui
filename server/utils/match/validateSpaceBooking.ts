...
export const validateSpaceBooking = (body: ObjectWithDateParts<string>): Record<string, string> => {
  ...
  if (dateIsBlank(body, 'arrivalDate')) {
    errors.arrivalDate = 'You must enter a arrival date'
    return errors
  }
  ...
  if (datetimeIsInThePast(arrivalDate)) {
    errors.arrivalDate = 'The arrival date must not be in the past'
  } else if (datetimeIsInThePast(departureDate, arrivalDate)) {
    errors.departureDate = 'The departure date must not before the arrival date'
  }
  return errors
}
