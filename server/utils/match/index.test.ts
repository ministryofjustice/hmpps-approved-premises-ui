  ...
  describe('occupancyViewSummaryListForMatchingDetails', () => {
    ...
    it('should call the correct row functions', () => {
      expect(occupancyViewSummaryListForMatchingDetails(totalCapacity, placementRequest, managerDetails)).toEqual([
        ...
        licenseExpiryDateRow(placementRequest.application as ApprovedPremisesApplication),
    ...
    it(`should generate the expected matching details with blank license expiry date when application's license-expiry date is not set`, () => {
      const placementRequestWithoutLicenseExpiry = {
        ...placementRequest,
      ...
      expect(
        occupancyViewSummaryListForMatchingDetails(totalCapacity, placementRequestWithoutLicenseExpiry, managerDetails),
      ).toEqual(expectedMatchingDetailsSummaryListItems(''))
    })
  })

  const expectedMatchingDetailsSummaryListItems = (expectedLicenseExpiryDate: string): Array<SummaryListItem> => {
    return [
      {
        key: {
      ...
      },
      {
        ...
        value: {
          text: expectedLicenseExpiryDate ? DateFormats.isoDateToUIDate(expectedLicenseExpiryDate) : '',
        },
      },
      {
        key: {
