...
import { AppealSessionData, ErrorsAndUserInput, RadioItem } from '@approved-premises/ui'
...
describe('placementAppealController', () => {
  ...
  const sessionData: AppealSessionData & { staffConflictOfInterestDetail: string } = {
  ...
  describe('confirm', () => {
    it('should render the confirmation form', async () => {
      ...
      await requestHandler(request, response, next)
      expect(response.render).toHaveBeenCalledWith('manage/premises/placements/changeRequests/appealConfirm', {
