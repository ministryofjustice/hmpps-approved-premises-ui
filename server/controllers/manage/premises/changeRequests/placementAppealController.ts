...
import { AppealJson, AppealSessionData, ObjectWithDateParts } from '@approved-premises/ui'
...
export default class PlacementAppealController {
  ...
  new(): RequestHandler {
    return async (req: Request, res: Response) => {
      ...
      return res.render('manage/premises/placements/changeRequests/appealNew', {
  ...
  newSave(): RequestHandler {
    return async (req: Request, res: Response) => {
      const { premisesId, placementId } = req.params
      const { body } = req as { body: AppealSessionData }
      try {
  ...
  confirm(): RequestHandler {
    return async (req: Request, res: Response) => {
      ...
      try {
        ...
        const placement = await this.premisesService.getPlacement({ token: req.user.token, premisesId, placementId })
        return res.render('manage/premises/placements/changeRequests/appealConfirm', {
  ...
  create(): RequestHandler {
    return async (req: Request, res: Response) => {
      ...
      try {
        ...
        const reasonDetailKey = `${appealReason}Detail` as keyof AppealJson
        requestJson[reasonDetailKey] = sessionData[reasonDetailKey]
        ...
        req.flash('success', {
          ...
          body: `This placement will remain visible under 'new and upcoming' until your appeal is progressed by the CRU.`,
        })
