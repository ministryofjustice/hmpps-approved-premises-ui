...
describe('NationalOccupancyController', () => {
  ...
  describe('index', () => {
    ...
    it('should render form with data populated from the query', async () => {
      ...
      expect(response.render.mock.calls[0][1]).toEqual({
        ...
        processedCapacity: processCapacity(nationalCapacity, postcode, apType),
        fromDate: '2025-12-16',
      ...
      expect(premisesService.getMultipleCapacity).toHaveBeenCalledWith('TEST_TOKEN', {
        ...
        premisesCharacteristics: apCriteria,
  ...
  describe('when errors are raised by the API', () => {
    beforeEach(() => {
      jest.spyOn(validationUtils, 'catchAPIErrorOrPropogate').mockReturnValue()
    })

    it('catches errors generated by the API', async () => {
      const apiError = new Error('API error')
      premisesService.getMultipleCapacity.mockImplementation(() => {
        throw apiError
      })

      const request = mockRequest({ arrivalDate: '16/12/2025' })

      await nationalOccupancyController.index()(request, response, next)

      expect(validationUtils.catchAPIErrorOrPropogate).toHaveBeenCalledWith(request, response, apiError)
    })
  })
})
