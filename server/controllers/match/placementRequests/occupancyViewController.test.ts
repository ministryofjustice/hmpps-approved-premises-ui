...
describe('OccupancyViewController', () => {
  ...
  describe('view', () => {
    ...
    it('should render the occupancy view template with errors', async () => {
      ...
      const expectedErrors = {
        arrivalDate: {
          text: 'You must enter a arrival date',
          attributes: {
  ...
  describe('bookSpace', () => {
    ...
    it(`should redirect to occupancy view and add errors messages when date validation fails`, async () => {
      ...
      expect(addErrorMessageToFlash).toHaveBeenCalledWith(request, 'You must enter a arrival date', 'arrivalDate')
      ...
      expect(response.redirect).toHaveBeenCalledWith(
        `${matchPaths.v2Match.placementRequests.spaceBookings.viewSpaces.index({ id: placementRequestDetail.id })}?${expectedParams}`,
      )
    })
  })
})
