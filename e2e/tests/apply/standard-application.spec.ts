...
test('Apply, assess, match and book an application for an Approved Premises with a release date', async ({
  ...
  const matchAndBookApplicationResult = await matchAndBookApplication({
    applicationId: id,
    page,
    apType,
    releaseType,
  ...
  await manageBooking({
    page,
    premisesName: matchAndBookApplicationResult.premisesName,
    datesOfPlacement: matchAndBookApplicationResult.newDatesOfPlacement,
  })
})
