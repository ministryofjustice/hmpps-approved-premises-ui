...
test('Record a successful appeal against a rejected application', async ({ page, person, assessor }) => {
  await signIn(page, assessor)
  const { id } = await createApplication({ page, person, applicationType: 'standard' }, false, true)
...
test('Record an unsuccessful appeal against a rejected application', async ({ page, person, assessor }) => {
  await signIn(page, assessor)
  const { id } = await createApplication({ page, person, applicationType: 'standard' }, false, true)
  await assessApplication({ page, assessor, person }, id, { acceptApplication: false })
